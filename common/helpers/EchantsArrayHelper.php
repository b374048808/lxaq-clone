...
class EchantsArrayHelper
{
    ...
    public static function lineOrBarInTime(
        ...
        $datas = call_user_func($function, $time['start'], $time['end'], self::$formats[$format]['sql']);
        ...
        foreach ($datas as $k => $data) {
            ...
            foreach ($all as &$item) {
                if (isset($data[$item])) {
                    $field = $seriesData[$k]['field'];
                    $seriesData[$k]['data'][] = $data[$item][$field] ?? 0;
                } else {
                    $seriesData[$k]['data'][] = 0;
                }
