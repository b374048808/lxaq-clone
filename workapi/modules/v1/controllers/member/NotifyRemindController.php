 ...
 * @LastEditTime: 2021-11-18 16:30:52
 * @Description: 
 */
 ...
 */
class NotifyRemindController extends OnAuthController
{
    /**
     * @var Worker
     */
    ...
    public function actionList($start = 0, $limit = 20){
        ...
        $model = NotifyMember::find()
            ...
            ->andWhere(['type' => Notify::TYPE_REMIND, 'member_id' => Yii::$app->user->identity->member_id])
            ->orderBy('id desc')
            ->offset($start)
            ->limit($limit)
            ->asArray()
    ...
    public function actionRealAll(){
        NotifyMember::updateAll(['is_read' => true, 'updated_at' => time()], ['member_id' => Yii::$app->user->identity->member_id,'type' => Notify::TYPE_REMIND, 'is_read' => false]);
