...
class NotifyService extends Service
{
     ...
     */
    public function createRemind($member_id,$target_id, $targetType, $action, $sender_id, $content)
    {
        $model = new Notify();
        ...
        $NotifyMember->notify_id = $model->attributes['id'];
        $NotifyMember->member_id = $member_id;
     ...
     * @param string $type
     */
    public function pullRemind($type = SubscriptionAlertTypeEnum::SYS)
    {

        // 查询最后的一条提醒时间
        $lastTime = Yii::$app->services->workerNotifyPullTime->getLastTime(Yii::$app->user->identity->member_id, Notify::TYPE_REMIND, $type);
        // 直接通过自己的关注去拉取消息
        $notifys = Notify::find()
        ...
        foreach ($notifys as $notify) {
            ...
            $NotifyMember->member_id = Yii::$app->user->identity->member_id;
            $NotifyMember->type = Notify::TYPE_REMIND;
            $NotifyMember->save();
        }
